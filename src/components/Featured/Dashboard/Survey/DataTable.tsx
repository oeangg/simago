...
export function SurveyDataTable({
  ...
  const exportSelectedRows = () => {
    try {
      ...
      const surveys = selectedRows
        .map((row) => {
          try {
            return getSurveyInFromRow(row);
          } catch (error) {
            console.error("Error getting survey from row:", error);
            return null;
          }
        })
        .filter((survey): survey is SurveyInColumnsProps => survey !== null);

      if (surveys.length === 0) {
        console.warn("No survey selected for export");
        return;
      }

      // Perbaikan: Map setiap vendor dengan index yang sesuai
      const csvData = surveys.map((dt) => {
        // Ambil data berdasarkan vendor yang sedang diproses
        // Helper function untuk format alamat

        return {
          "No Survey": dt.surveyNo,
          "Tgl Survey": formatDate(dt.surveyDate),
          "Tgl Pekerjaan": formatDate(dt.workDate),
          Customer: dt.customer.name,
          "Jumlah Items": dt.surveyItems.length.toString(),
        };
      });
      ...
      exportToCSV(csvData, "data-survey");
      toast.success(`${csvData.length} survey berhasil dieksport`);
    } catch (error) {
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          ...
          >
            <Link href="/dashboard/survey/add">
